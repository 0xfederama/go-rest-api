...
// render 'v' as JSON and writes it as a response into w.
...
func (ts *taskServer) taskHandler(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "/task/" {
		...
		if req.Method == http.MethodPost {
			...
			http.Error(w, fmt.Sprintf("expect method GET, POST, DELETE at /task/, got %v", req.Method), http.StatusMethodNotAllowed)
			return
		}
	} else {
